...
template<class CType> class CHashMap
{
	private:
		typedef CHashMap<CType> TMyType;
		CType* m_pBuffer;
		size_t m_iLength;
		size_t m_iCapacity;
	...
	public:
		class CRange
		{
			...
			private:
				...
				CRange(CHashMap& cVector, size_t iRangeStart, size_t iRangeLength): m_cVector(cVector)
				{
		...
		CHashMap(const CHashMap& cVector)
		{
			m_pBuffer = NULL;
			m_iCapacity = 0;
			m_iLength = 0;
